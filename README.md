...
pip install phonenumbers pydantic jsonschema jsonpath-ng pycountry
```

## How to Run

You can run the engine via the command line interface `main.py`. Provide at least one source file (`--csv` or `--github`), and optionally a config file (`--config`) to reshape the output.

### 1. Default Canonical Output
To run the pipeline and see the raw canonical profile with default schema:
```bash
python3 main.py --csv sample.csv --github sample_github.json
```
...
```bash
python3 main.py --csv sample.csv --github sample_github.json --config custom_config.json
```

### 3. Save to file
To save the output to a JSON file rather than printing to stdout:
```bash
python3 main.py --csv sample.csv --github sample_github.json --config custom_config.json --out result.json
```
